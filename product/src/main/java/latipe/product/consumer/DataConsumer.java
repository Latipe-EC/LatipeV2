        ...
        try {
            if (consumerRecord != null) {
                ...
                if (usingLog.isPresent()) {
                    var prods = productRepository.findAllByIdsAndStoreId(usingLog.get().getItems().stream().map(
                            UsingItem::getProductId).toList(), usingLog.get().getStoreId());

                    for (var item : usingLog.get().getItems()) {
                        var prod = prods.stream().filter(p -> p.getId().equals(item.getProductId())).findFirst();
                        ...
                        if (prod.isPresent()) {
                            ...
                            if (classification.isPresent()) {
                                classification.get().setQuantity(classification.get().getQuantity() + item.getQuantity());
                            } else {
                                // not found classification
