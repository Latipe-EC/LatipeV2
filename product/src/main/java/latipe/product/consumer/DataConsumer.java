                    ...
                    if (prod.isPresent()) {
                        ...
                        if (classification.isPresent()) {
                            if (classification.get().getQuantity() >= item.quantity()) {
                                classification.get()
                                    .setQuantity(
                                        classification.get().getQuantity() - item.quantity());

                                orderLog.getItems().add(
                                    new UsingItem(item.productId(), item.optionId(),
                                        item.quantity()));
                            } else {
            ...
            if (consumerRecord != null) {
                ...
                if (usingLog.isPresent()) {
                    ...
                    for (var item : usingLog.get().getItems()) {
                        ...
                        if (prod.isPresent()) {
                            var classification = prod.get().getProductClassifications()
                                .stream().filter(c -> c.getId().equals(item.getOptionId()))
                                .findFirst();

                            if (classification.isPresent()) {
                                classification.get()
                                    .setQuantity(
                                        classification.get().getQuantity() + item.getQuantity());
                            } else {
                                // not found classification
