...
import latipe.auth.constants.CONSTANTS;
...
public class AuthController {
    ...
    public CompletableFuture<LoginResponse> createAuthenticationToken(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            // create new session
            final String newSid = GenTokenUtils.encodeSession(user.getEmail(),
                CONSTANTS.ENCRYPTION_KEY);

            // save sid to cache
            tokenCache.saveSid(newSid, user.getId());
            ...
            return LoginResponse.builder().accessToken(accessToken).refreshToken(refreshToken)
                ...
                .displayName(user.getDisplayName()).phone(user.getPhoneNumber())
                .email(user.getEmail())
    ...
    public CompletableFuture<RefreshTokenResponse> refreshAuthenticationToken(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            var username = jwtUtil.checkToken(refreshToken, "refresh-token");
            var user = getUser(username);
            if (user.getIsDeleted()) {
                throw new UnauthorizedException("Your account has been deleted");
            }

            var sessionData = tokenCache.getSid(request.getHeader("Sid"));

            if (sessionData != null) {
                if (!sessionData.equals(user.getId())) {
    ...
    public CompletableFuture<UserCredentialResponse> validateToken(
        ...
        var username = jwtUtil.checkToken(accessToken.token(), "access-token");
        var userCache = tokenCache.getUserDetails(genKeyCacheToken(accessToken.token(), username));

        var sid = request.getHeader("Sid");
            ...
            try {
                if (jwtUtil.validateToken(accessToken.token(), user)) {
                    ...
                    return UserCredentialResponse.builder().email(user.getEmail())
                        .phone(user.getPhoneNumber()).id(user.getId())
    ...
    public Void verifyAccount(@Valid @RequestBody ForgotPasswordRequest input,
        ...
        LOGGER.info(gson.toJson(
            LogMessage.create("Forgot password request from user %s".formatted(input.email()),
                request,
                getMethodName())
        ));
    ...
    private User getUser(String username) {
        ...
        Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
            Aggregation.lookup(mongoTemplate.getCollectionName(Role.class), "roleId", "_id",
                "roles"),
            Aggregation.addFields()
                .addFieldWithValue("role", ArrayOperators.arrayOf("roles").elementAt(0)).build());
        var userRaw = mongoTemplate.aggregate(aggregation,
