...
public class PaymentService {
    ...
    public CompletableFuture<CapturedPaymentResponse> capturePayment(
        ...
        log.info(gson.toJson(
            LogMessage.create(
                "Capture payment with order id: %s".formatted(completedPayment.orderId()),
    ...
    public CompletableFuture<Void> validWithdrawPaypal(
        ...
        return CompletableFuture.supplyAsync(
            () -> {
                var tokenWithdraw = gson.fromJson(
                    TokenUtils.decodeToken(input.token(), ENCRYPTION_KEY),
                    TokenWithdraw.class);
                ...
                try {
                    ...
                    log.error(
                        "Cannot withdraw amount with withdraw id: {}, update status to cancel",
                        withdraw.getId());
                    withdraw.setWithdrawStatus(EWithdrawStatus.CANCELLED);
                    withdrawRepository.save(withdraw);
                    throw new BadRequestException("Cannot withdraw amount");
                ...
                try {
                    req = requestTransferMoney(withdraw.getEmailRecipient(),
                        withdraw.getAmount()
                            .divide(new BigDecimal(23000), 2, RoundingMode.CEILING));
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                ...
                try {
                    ...
                    storeClient.updateBalance(hash,
                        new UpdateBalanceStoreRequest(getUserId(request),
    ...
    @Async
    public CompletableFuture<TotalAmountResponse> getTotalAmount(TotalAmountRequest input,
        HttpServletRequest request) {
        log.info(gson.toJson(
            LogMessage.create("Get total amount with order ids: [%s]".formatted(StringUtils.join(
                input.orderIds(), ", ")), request, getMethodName())));
        ...
        return CompletableFuture.supplyAsync(
            () -> {
                ...
                var totalAmount = payments.stream().map(Payment::getAmount)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

                log.info("Get total amount with order ids: [%s] successfully".formatted(
                    StringUtils.join(input.orderIds(), ", ")));
    ...
    public CompletableFuture<CheckPaymentOrderResponse> checkPaymentInternal(String orderId,
        ...
        return CompletableFuture.supplyAsync(
            () -> {
                ...
                log.info(
                    "Check payment internal with order id: %s successfully".formatted(orderId));
                return CheckPaymentOrderResponse.fromModel(payment);
            }
        );
    }
